...
def expand_key(key, rounds):
    ...
    for round in range(rounds):
        ...
        last_column_rcon_step = [
            last_column_sbox_step[i] ^ rcon[round][i]
...
def dec(key, data):
    ...
    for grid in grids:
        ...
        shift_rows_step = [
            rotate_row_left(add_sub_key_step[i], -1 * i) for i in range(4)
        ]
        sub_bytes_step = [
    ...
    for round in range(9, 0, -1):
        ...
        for grid in grids:
            ...
            mix_column_step = mix_columns(mix_column_step)
            shift_rows_step = [
                rotate_row_left(mix_column_step[i], -1 * i) for i in range(4)
            ]
            sub_bytes_step = [
                [reverse_lookup(val) for val in row] for row in shift_rows_step
            ]
...
def client_program():
    key = input("Enter the value of key (16 character long (A-Z): ")
    key = bytes(key, "utf-8")
